...
public class BankAccountUsingArrayList {
	...
	private static void createAccount() {
		...
		arrayList.add(newAccount);   //null 인 방에 객체를 저장. 
	...
	private static void accountList() {
		...
		//List의 각방의 null아닌 경우 , 객체의 필드의 값을 출력. 
		for ( int i = 0 ; i <  arrayList.size(); i++) {
			//각 방의 객체를 담는 변수를 선언 
			
			Account account = arrayList.get(i);    // 0 ~ 99 방의 객체를 account 참조 변수에 담는다. 
			if (account != null) {
	...
	//List에서 Account 객체 내의 ano[계좌번호] 와 동일한 계좌를 찾아서 찾은 객체를 리턴으로 돌려줌. 
	//예금, 출금할때 공통코드 , 여러 메소드에서 중복 사용 될때 별도의 메소드를 생성해서 중복코드를 간단하게 만들어 준다. 
	private static Account findAccount(String ano) {
		Account account = null ; 
		//코드 작성 
		for (int i = 0 ; i < arrayList.size() ; i++) {   
			if (arrayList.get(i) != null) {		//List방의 값이 null이 아닐 경우에 객체의 ano[계좌] 번호. 
				//각 객체의 방의 ano 를 담는 변수 선언. 
				String dbAno = arrayList.get(i).getAno(); //List의 각 방에 저장된 객체의 ano를 dbAno 변수에 할당. 
				if (dbAno.equals(ano)) {
					account = arrayList.get(i);  //account참조변수에 객체의 주소정보를 담는다. 
					break;
				}
			}
		}
