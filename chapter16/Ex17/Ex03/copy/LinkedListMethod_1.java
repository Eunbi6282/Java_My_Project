package chapter16.Ex17.Ex03.copy;
...
public class LinkedListMethod_1 {
	public static void main(String[] args) {
		...
		linkedlist2.addAll(linkedlist1); 	//aLsit1에 저장된 값을 복사해서 aList2에 추가함
		...
		linkedlist3.set(1, 5);	// 1번방번호의 값을 5로 수정해라
		linkedlist3.set(3, 6);
		// aList3.set(4, 7); -> 오류발생 (4번방은 존재하지 않음)
		System.out.println(linkedlist3);
		
		//6. remove (int index) : 특정 방번호의 값을 삭제,
		linkedlist3.remove(1); // 방번호 1번에 있는 값을 삭제, 방번호를 삭제하는거임!!!!
			// 삭제되면 뒤에 있는 값이 앞으로 땡겨옴
		System.out.println(linkedlist3); // 1번방의 값이 삭제되고 뒤의 값들이 앞으로 당겨짐
		
		// 7. remove(Object O) :값으로 삭제
		linkedlist3.remove(new Integer(2)); //값으로 삭제를 할 수 있다. , 값 2가 삭제됨
		System.out.println(linkedlist3);
		...
		System.out.println(linkedlist3.isEmpty()); //true -> 앞에서 clear사용해끼 때문에
		...
		System.out.println(linkedlist3); //List일땐 toString()이 재정의 되어있기 때문에 값을 출력한다.
		
		Object[]objects = linkedlist3.toArray(); //기본적으로 toArray()메서드는 Object타입으로 리턴이 됨/ List를 배열타입으로 
			// 다운캐스팅 필요
