...
동기화 : 여러 쓰레드가 공유된 필드를 접근할 때 하나의 쓰레드가 완료되면 다른 쓰레드에서 접근이 가능
	...
	클래스 내부에 여러개의 동기화 메서드가 존재할 경우 : 쓰레드가 접근하는 키가 모두 this이다.
	여러개의 동기화 메서드 내에서 멀티 쓰레드를 처리하고자 할 경우 : 동기화 블락을 생성 후 키를 다르게 설정.
*/
class A{}
class B{}
class C{}
...
// 동기화 블럭은 Key값을 임의로 할당 가능하다.
	synchronized void abc() {		//여러 쓰레드가 접근할 때 동시성 문제를 방지하기 위해서 동기화 처리 필요
	 ...
	 void bcd() {		//여러 쓰레드가 접근할 때 동시성 문제를 방지하기 위해서 동기화 처리 필요
