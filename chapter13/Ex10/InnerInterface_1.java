...
// 이너 인터페이스 (InnerInterface) : 클래스 내부의 인터페이스가 선언, 외부클래스 간단하게 사용할 목적으로
	//클래스 내부에 선언
...
public class InnerInterface_1 {
	...
	public static void main(String[] args) {
		...
		C c = new C(); 		// 자신의 객체통해 직접 호출
		c.bcd();
		
		// 2. 익명 이너클래스로 출력(자식클래스 없이 출력)
		A.B b = new A.B() {		//A.B : A클래스 내부의 인터페이스ㅅ
			@Override
			public void bcd() {
				System.out.println("익명이너클래스로 객체 생성후 출력");
			}		
		};	
		b.bcd();
