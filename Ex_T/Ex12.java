...
//		컴퓨터와 독자 사이의 가위 바위 보 게임을 만들어보자. 예시는 다음
...
//		if(str[n].equals("바위")) //컴퓨터가 낸 것이 "바위"인지 비교하는 
		...
		String str[] = {"가위", "바위", "보"};
		String uSelect, comSelect;//uSelect : 당신이 낸 값(스캐너로 인풋), comSelect : 컴퓨터가 낸값(Math.random으로 인풋)
		int n;	//컴퓨터가 낸 값 (0,1,2)
		...
		do {
			...
			n = (int)(Math.random()*3);	//n은 컴퓨터가 내는 값 : 0,1,2
			comSelect = str[n]; //컴퓨터가 내는 값 : n이 0일때 -> 가위, 1 -> 바위, 2 -> 보
			...
			if(uSelect.equals("가위")) {		//사용자가 가위일 때
				if (comSelect.equals("가위")) {
					...
					System.out.println("사용자 = " + uSelect + ", 컴퓨터 = " + comSelect + " 사용자가 이겼습니다.");
				}
